...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, key_pad_mask: torch.Tensor):
        ...
        context_length, batch, _ = x.shape
...
class CLIP(nn.Module):
    ...
    def forward(self, image, text, attention_mask):
        ...
        attention_mask= torch.from_numpy( np.where((attention_mask.cpu() == 0) | (attention_mask.cpu() == 1), attention_mask.cpu() ^ 1, attention_mask.cpu())).to("cuda")
