...
class CLIP(nn.Module):
    ...
    def encode_image(self, image):
        x = self.visual(image)
        # x_proj = x @ self.image_projection
    ...
    def encode_text(self, text, key_pad_mask):
        ...
        x = self.transformer(x, key_pad_mask)
        x = x.permute(1, 0, 2)  # LND -> NLD
        x = self.ln_final(x)

        # x.shape = [batch_size, n_ctx, transformer.width]
        # take features from the eot embedding (eot_token is the highest number in each sequence)
        x_pooled = x[torch.arange(x.shape[0]), text.argmax(dim=-1)] #@ self.text_projection

        return x, x_pooled

    def forward(self, image, text, key_pad_mask):
        image_embeds, image_embed = self.encode_image(image)
        text_embeds, text_embed = self.encode_text(text, key_pad_mask)
...
def vit_small_patch16_224(**kwargs):
    model = VisionTransformer(
        patch_size=16, embed_dim=384, depth=12, num_heads=12, mlp_ratio=4, global_pool='', qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model

def vit_base_patch16_224(**kwargs):
    model = VisionTransformer(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, global_pool='', qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model
...
def vit_large_patch16_224(**kwargs):
    model = VisionTransformer(
        patch_size=16, embed_dim=1024, depth=24, num_heads=16, mlp_ratio=4, global_pool='', qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model
