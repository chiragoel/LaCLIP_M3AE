...
from model_mmae_laclip import MMAELaCLIP
from train_mmae_laclip import train
...
def set_args():
    ...
    parser.add_argument('--model', default='MV_MMAE_CLIP', type=str, help='the model name', choices=['MV_CLIP','MV_MMAE_LaCLIP'])
...
def main():
    ...
    if args.model == 'MV_MMAE_LaCLIP':
        ...
        model = MMAELaCLIP(args, device=device, config_updates=model_config,  layers=args.layers, num_classes=2, model_type='base', global_pool='org')
    else:
