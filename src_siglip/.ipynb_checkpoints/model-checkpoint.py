...
class LACLIP(nn.Module):
    ...
    def forward(self, inputs, labels):
        print("pixel val, input id, attn_mask shape: ", inputs['pixel_values'].shape, inputs['input_ids'].shape, inputs['attention_mask'].shape)
        output = self.model(inputs['pixel_values'], inputs['input_ids'], inputs['attention_mask']) 
        # text_features = output['text_model_output']['last_hidden_state']
        # image_features = output['vision_model_output']['last_hidden_state']
