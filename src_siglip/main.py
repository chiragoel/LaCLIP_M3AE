...
from model import MV_CLIP, LACLIP, SIGLIP
...
def set_args():
    ...
    parser.add_argument('--lr_scheduler', default='linear', type=str, help='Learning rate scheduler', choices=['linear', 'cosine'])
    parser.add_argument('--modelname', default='model', type=str, help='experiment model checkpoint name')
...
def main():
    ...
    project_names = {
        'MV_CLIP': "MMSD2.0",
        'LACLIP': "MMSD2.0",
        'SIGLIP': "SIGCLIP_MMSD2_jerome"
    }
    wandb.init(
        project=project_names[args.model],
        notes="mm",
        tags=[args.tag],
        # id=[args.modelname]
        config=vars(args),
    ...
    if args.model == 'MV_CLIP':
        ...
        # print("processor siglip: ", processor)
        model = SIGLIP(args)
