        ...
        if args.model == 'MV_CLIP':
            ...
            if args.lr_scheduler == "cosine":
                scheduler = get_cosine_schedule_with_warmup(optimizer, num_warmup_steps=int(args.warmup_proportion * total_steps),
                                                    num_training_steps=total_steps)
        elif args.model == 'LACLIP' or args.model == 'SIGLIP':
            clip_params = list(map(id, model.model.parameters()))
            base_params = filter(lambda p: id(p) not in clip_params, model.parameters())
    ...
    for i_epoch in trange(0, int(args.num_train_epochs), desc="Epoch", disable=False):
        ...
        for step, batch in enumerate(iter_bar):
            ...
            if args.model == 'SIGLIP':
                inputs = processor(text=text_list, images=image_list, padding='max_length', truncation=True, max_length=args.max_len, return_tensors="pt").to(device)
                labels = torch.tensor(label_list).to(device)
                inputs['attention_mask'] = torch.where(inputs['input_ids'] > 1, 1, 0).to(device)
                # print(inputs['input_ids'][0], inputs['input_ids'][-1])
                # print("siglip inputs- ", inputs.keys(), "pixels shape: ", inputs['pixel_values'].shape, "text shape: ", inputs['input_ids'].shape, "attention mask shape: ", inputs['attention_mask'].shape)
        ...
        # print({'dev_acc': dev_acc, 'dev_f1': dev_f1, 'dev_precision': dev_precision, 'dev_recall': dev_recall})
        ...
        if dev_acc > max_acc:
            ...
            torch.save(model_to_save.state_dict(), os.path.join(path_to_save, f'{args.modelname}.pt')) # model_laion_LAclip_test_run
